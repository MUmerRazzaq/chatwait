...
**Independent Test**: Can be fully tested by sending a single POST request to /chat/wait with a user message and verifying a complete response is returned. Delivers immediate value as a working chatbot.

**Acceptance Scenarios**:

1. **Given** the service is running, **When** a user sends "Hello, how are you?" to /chat/wait, **Then** the system returns a complete conversational response within 5 seconds
2. **Given** the service is running, **When** a user sends a message to /chat/wait, **Then** the response includes the complete generated text with no truncation
...
**Independent Test**: Can be fully tested by establishing an SSE connection to /chat/streaming, sending a message, and verifying that tokens arrive incrementally before the full response completes. Delivers enhanced UX independently.

**Acceptance Scenarios**:

1. **Given** the service supports SSE, **When** a user connects to /chat/streaming and sends a message, **Then** the user begins receiving response tokens within 2 seconds
2. **Given** the chatbot is generating a streaming response, **When** tokens are generated, **Then** each token is sent to the client immediately as an SSE event
3. **Given** a streaming response is in progress, **When** the generation completes, **Then** a final event indicates the stream has ended
4. **Given** the user has an active SSE connection, **When** the user sends multiple messages, **Then** each message triggers a new streaming response with clear boundaries between responses
5. **Given** a streaming response is in progress, **When** the user sends a follow-up message, **Then** the system supports multi-turn conversation context just like synchronous mode
...
1. **Given** a streaming connection drops mid-response, **When** the client reconnects to /chat/streaming, **Then** the client can resume the conversation without re-sending previous messages
2. **Given** the client reconnects after a disconnection, **When** the client provides the last received message/token ID, **Then** the system resumes streaming from that point (or retries gracefully)
...
- **FR-001**: System MUST expose a /chat/wait endpoint that accepts user messages via POST request and returns a complete generated response in a single payload
- **FR-002**: System MUST expose a /chat/streaming endpoint that accepts user messages and streams response tokens incrementally using Server-Sent Events (SSE)
...
- **FR-005**: /chat/wait endpoint MUST return responses with HTTP 200 status code for successful requests and appropriate error codes (400, 500, 503) for failures
- **FR-006**: /chat/streaming endpoint MUST establish SSE connections and send response tokens as individual events with appropriate event formatting
...
- **FR-008**: System MUST implement message ID or sequence number tracking to support resumption after reconnection in streaming mode
- **FR-009**: System MUST provide clear error messages that explain what went wrong without exposing internal system details or sensitive information
...
- **FR-013**: System MUST respond to malformed requests with appropriate HTTP status codes (400 for client errors, 500 for server errors)
- **FR-014**: System MUST support UTF-8 encoding for all text input and output, including emojis and special characters
- **FR-015**: /chat/streaming endpoint MUST send a clear end-of-stream event when response generation completes
- **FR-016**: System MUST maintain consistent response quality and behavior between /chat/wait and /chat/streaming modes (same chatbot logic)
- **FR-017**: System MUST be extensible to allow future additions (authentication, persistence, integrations) without breaking existing endpoint contracts
...
- **SSE Event**: Represents a single server-sent event in streaming mode. Contains: event type (token, end, error), data payload (token text or metadata), optional event ID for resumption.

## Success Criteria _(mandatory)_

### Measurable Outcomes

- **SC-001**: Users can send a message to /chat/wait and receive a complete response within 5 seconds for messages up to 500 characters
- **SC-002**: Users can connect to /chat/streaming and begin receiving response tokens within 2 seconds of sending a message (time-to-first-token)
